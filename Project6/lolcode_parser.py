...
def build_parser(possible_tokens):
    ...
    @pg.production('statement : WTF QUESTION_MARK expression newlines opt_case opt_omgwtf OIC')
    def case_statement(p):
        condition = p[2]
        case_statements = p[4]
        omgwtf_opt_case, omgwtf = p[5]
        case_statements = [condition] + case_statements + omgwtf_opt_case
        return WTFStatement(case_statements, omgwtf)
    @pg.production('opt_case : ')
    @pg.production('opt_case : OMG expression newlines code_block opt_case')
    def opt_case(p):
        return [p[1], p[3]] + p[4] if p else []
    @pg.production('opt_omgwtf : ')
    @pg.production('opt_omgwtf : OMGWTF newlines code_block')
    def omgwtf(p):
        if p:
            return [p[2]], True
        else:
